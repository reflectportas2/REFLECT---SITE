      ...
      <section id="projetos" className="px-6 py-20 md:px-12 lg:px-20">
        <div className="mx-auto max-w-7xl">
          <div className="mb-12 max-w-2xl space-y-4">
            <p className="text-sm font-semibold uppercase tracking-[0.3em] text-[#8A6A4F]">Por que escolher a Reflect</p>
        ...
        Elegância e personalidade para cada ambiente.
          ...
          <div className="grid gap-5 md:grid-cols-3">
            {[
              ...
              { icon: ShieldCheck, title: "Acabamento premium", text: "Materiais selecionados e visual sofisticado para alto padrão." },
              { icon: MessageCircle, title: "Atendimento consultivo", text: "Você conversa com especialista antes de fechar o orçamento." },
            ].map((item) => (
              <div key={item.title} className="rounded-3xl border-0 bg-white/70 p-8 shadow-sm">
                <div className="space-y-5">
                  <div className="inline-flex rounded-2xl bg-[#EFE4D6] p-4"><item.icon className="text-[#6B4F3B]" size={26} /></div>
                  <h3 className="text-xl font-semibold">{item.title}</h3>
                  <p className="leading-7 text-[#6B6258]">{item.text}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid items-center gap-10 lg:grid-cols-2">
            ...
            <div className="rounded-3xl bg-white/70 p-6 shadow-sm">
              <div className="space-y-4">
                {["1. Informe o móvel ou porta desejada", "2. Envie medidas ou imagens do espaço", "3. Receba uma proposta personalizada"].map((step) => (
